...
var start, isBlink, isLight, isRun, isShow, isWarned, handler, latency, stopBy, delay, audioRemind, audioEnd, dolphin, ocean, newAudio, soundToggle, show, adjust, toggle, reset, blink, count, run, resize, remainingTime, updateDolphin, resetDolphin, startSwim, stopSwim;
start = null;
isBlink = false;
isLight = true;
isRun = false;
...
updateDolphin = function(remaining){
  var total, progress, width, dolphinWidth, startPos, travel, offset;
  remaining == null && (remaining = remainingTime());
  if (dolphin == null) {
    return;
  }
  ...
  width = $(window).width();
  dolphinWidth = dolphin.outerWidth();
  startPos = -dolphinWidth;
  travel = width + dolphinWidth * 2;
  offset = startPos + progress * travel;
  return dolphin.css('--swim-offset', offset + "px");
};
resetDolphin = function(){
  return updateDolphin(delay);
};
startSwim = function(){
  if (ocean) {
    ocean.addClass('swimming');
  }
  return updateDolphin();
};
stopSwim = function(){
  if (ocean) {
