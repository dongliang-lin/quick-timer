...
var start, isBlink, isLight, isRun, isShow, isWarned, handler, latency, stopBy, delay, totalDelay, audioRemind, audioEnd, newAudio, soundToggle, show, adjust, toggle, reset, blink, count, run, updateIncense, resize;
start = null;
isBlink = false;
isLight = true;
isRun = false;
...
updateIncense = function(remaining){
  var line, ratio;
  line = $('#incense-line');
  if (!line.length) {
    return;
  ...
  ratio = Math.max(remaining, 0) / totalDelay;
  ratio = Math.min(ratio, 1);
  return line.width(ratio * 100 + "%");
};
resize = function(){
  var tm, w, h, len;
  tm = $('#timer');
